...
最近業務で暗号に触れる機会があり，専攻していた内容を学びなおしました．
本シリーズでは，HTTPS や SSH，Web API の署名検証のような題材に繋げながら，基礎から暗号の仕組みを整理します．
本稿では，群・環・体から離散対数問題までを扱います．
...
この「一定の範囲で値が折り返す」感覚は，時計の時刻計算や，配列インデックスをリング状に扱う実装に近いものです．
...
これは $\mathrm{GF}(p)$ の乗法群の位数が $p - 1$ であることの帰結で，モジュロ逆元の計算などに役立ちます (RSA では，より一般なオイラーの定理や中国剰余定理が本筋になります)．
...
有限体 $\mathrm{GF}(p)^*$ の文脈では，次のように書けます．
